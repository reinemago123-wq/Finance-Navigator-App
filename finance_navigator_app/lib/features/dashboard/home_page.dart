...
class _HomePageState extends State<HomePage> {
              ...
              child: Column(
                ...
                children: [
                    ...
                    Expanded(
                      child: GlassCard(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text('Income',
                                style: AppText.label.copyWith(color: AppColors.income)),
                            const SizedBox(height: Sp.md),
                            const Text('\$4,500', style: AppText.money),
                            const SizedBox(height: Sp.sm),
                            Text('+12% vs last month',
                                style: AppText.caption.copyWith(color: AppColors.income)),
                          ],
                        ),
                      ),
                    ),
                    ...
                    Expanded(
                      child: GlassCard(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text('Expenses',
                                style: AppText.label.copyWith(color: AppColors.expense)),
                            const SizedBox(height: Sp.md),
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      _quickAction(icon: Icons.add_circle_outline,
                          label: 'Income',
                          onTap: () => _push(AddTransactionPage(initialType: TxnType.income))),
                      _quickAction(icon: Icons.remove_circle_outline,
                          label: 'Expense',
                          onTap: () => _push(AddTransactionPage(initialType: TxnType.expense))),
                      _quickAction(icon: Icons.event_note_outlined,
                          label: 'Bill',
                          onTap: () => _push(AddTransactionPage(initialType: TxnType.bill))),
                      _quickAction(icon: Icons.savings_outlined,
                          label: 'Savings',
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text('Vacation Fund',
                                style: AppText.body.copyWith(
                                    fontWeight: FontWeight.w600, color: AppColors.onDark)),
                            const SizedBox(height: Sp.sm),
                            Text('\$6,500 / \$10,000',
                  ...
                  const SizedBox(height: Sp.md),
                  _txnItem(icon: Icons.shopping_bag_outlined, label: 'Grocery Store',
                      date: 'Today at 2:30 PM', amount: '-\$45.99', isExpense: true),
                  const SizedBox(height: Sp.md),
                  _txnItem(icon: Icons.work_outline, label: 'Salary Deposit',
                      date: 'Yesterday', amount: '+\$3,500.00', isExpense: false),
                  const SizedBox(height: Sp.md),
                  _txnItem(icon: Icons.local_cafe_outlined, label: 'Coffee Shop',
                      date: 'Apr 19', amount: '-\$5.50', isExpense: true),
                  const SizedBox(height: Sp.xl),
  ...
  Widget _balanceItem({required String label, required String amount, required Color color}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: AppText.caption.copyWith(color: AppColors.onDark.withOpacity(0.60))),
      const SizedBox(height: Sp.sm),
  ...
  Widget _budgetItem({required String label, required double spent, required double budget,
      ...
      child: Column(children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label, style: AppText.body.copyWith(color: AppColors.onDark)),
            const SizedBox(height: Sp.xs),
        ...
        ClipRRect(
          ...
          child: LinearProgressIndicator(
            ...
            backgroundColor: AppColors.onDark.withOpacity(0.10),
            valueColor: AlwaysStoppedAnimation<Color>(pct > 0.8 ? AppColors.expense : color),
          ),
        ),
      ]),
    );
  ...
  Widget _txnItem({required IconData icon, required String label,
      required String date, required String amount, required bool isExpense}) {
    return GlassCard(
      padding: const EdgeInsets.all(Sp.md),
      child: Row(children: [
        Container(
          width: 44, height: 44,
          decoration: BoxDecoration(
            color: (isExpense ? AppColors.expense : AppColors.income).withOpacity(0.15),
            borderRadius: BorderRadius.circular(Rd.md),
          ),
          child: Icon(icon, color: isExpense ? AppColors.expense : AppColors.income, size: 22),
        ),
        const SizedBox(width: Sp.md),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _billItem({required String provider, required String amount,
      required String dueDate, required String status, required IconData icon}) {
